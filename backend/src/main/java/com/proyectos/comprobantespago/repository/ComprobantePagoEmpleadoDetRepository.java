...
public interface ComprobantePagoEmpleadoDetRepository
        extends JpaRepository<ComprobantePagoEmpleadoDet, ComprobantePagoEmpleadoDet.ComprobantePagoEmpleadoDetId> {

    /**
     * Buscar detalles por comprobante
     */
    List<ComprobantePagoEmpleadoDet> findByCodCiaAndCodEmpleadoAndNroCp(Long codCia, Long codEmpleado, String nroCp);

    /**
     * Buscar detalles por partida
     */
    List<ComprobantePagoEmpleadoDet> findByCodCiaAndIngEgrAndCodPartida(Long codCia, String ingEgr, Long codPartida);

    /**
     * Buscar detalles ordenados por secuencia
     */
    @Query("SELECT d FROM ComprobantePagoEmpleadoDet d WHERE d.codCia = :codCia AND d.codEmpleado = :codEmpleado AND d.nroCp = :nroCp ORDER BY d.sec")
